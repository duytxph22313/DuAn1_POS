...
public class ViewThuongHieu extends javax.swing.JPanel {
    ...
    private ThuongHieuServiceImpl service = new ThuongHieuServiceImpl();
    private List<ThuongHieu> listTH = new ArrayList<>();

    public ViewThuongHieu() {
        initComponents();
        tblTH.setModel(dtmTH);
        Object[] header = {"Mã", "Tên"};
        dtmTH.setColumnIdentifiers(header);
        listTH = service.getAll();
        loadDataTable(listTH);
    }

    private void loadDataTable(List<ThuongHieu> list) {
        dtmTH.setRowCount(0);
        for (ThuongHieu th : list) {
            dtmTH.addRow(th.toDataRow());
        }
    }
    ...
    private void btnThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThemActionPerformed
        ...
        try {
            JOptionPane.showMessageDialog(this, this.service.create(th));
        } catch (Exception ex) {
            Logger.getLogger(ViewThuongHieu.class.getName()).log(Level.SEVERE, null, ex);
        }
        listTH = service.getAll();
        loadDataTable(listTH);;
