...
public class ViewChatLieu extends javax.swing.JPanel {
    ...
    public ViewChatLieu() {
        initComponents();
        tblTH.setModel(dtmCL);
        Object[] header = {"Mã", "Tên"};
        dtmCL.setColumnIdentifiers(header);
        listCL = service.getAll();
        loadDataTable(listCL);
    }

    private void loadDataTable(List<ChatLieu> list) {
        dtmCL.setRowCount(0);
        for (ChatLieu obj : list) {
            dtmCL.addRow(obj.toDataRow());
        }
    }
    ...
    private void btnThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThemActionPerformed
        ...
        try {
            JOptionPane.showMessageDialog(this, this.service.create(cl));
        } catch (Exception ex) {
            Logger.getLogger(ViewChatLieu.class.getName()).log(Level.SEVERE, null, ex);
        }
        listCL = service.getAll();
        loadDataTable(listCL);
